...
AWSConfig.region = process.env.REACT_APP_REGION

// Creates a CognitoAuth instance
const createCognitoAuth = () => {
  const appWebDomain = process.env.REACT_APP_USER_POOL_BASE_URI.replace('https://', '').replace('http://', '')
  const auth = new CognitoAuth({
    UserPoolId: process.env.REACT_APP_USER_POOL,
    ClientId: process.env.REACT_APP_CLIENT_ID,
    AppWebDomain: appWebDomain,
    TokenScopesArray: appConfig.tokenScopes,
    RedirectUriSignIn: process.env.REACT_APP_CALLBACK_URI,
    RedirectUriSignOut: process.env.REACT_APP_SIGNOUT_URI
  })
  return auth
}
...
const getCognitoSignInUri = () => {
  ...
  const signinUri = `${process.env.REACT_APP_USER_POOL_BASE_URI}/login?response_type=code&client_id=${process.env.REACT_APP_CLIENT_ID}&redirect_uri=${process.env.REACT_APP_CALLBACK_URI}`
  return signinUri
}
